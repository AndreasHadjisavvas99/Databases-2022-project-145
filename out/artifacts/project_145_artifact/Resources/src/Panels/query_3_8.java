...
public class query_3_8 {
    public JPanel query_3_8;
        ...
        btnFind.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    ps = c.prepareStatement("SELECT r.first_name,r.last_name,r.researcher_id, COUNT(pwod.project_id) as total from researcher as r JOIN works_on_project as wp ON r.researcher_id=wp.researcher_id JOIN projects_wo_deliverables as pwod ON wp.project_id=pwod.project_id GROUP BY r.Researcher_id having COUNT(pwod.project_id) > 4 ORDER BY total  DESC");
                    ResultSet rs = ps.executeQuery();

                    if(rs.next()) {
                        lblRe1.setText(rs.getString("first_name") + " " + rs.getString("last_name"));
                        lblCount1.setText(rs.getString("total"));
                    }
                    if(rs.next()) {
                        lblRe2.setText(rs.getString("first_name") + " " + rs.getString("last_name"));
                        lblCount2.setText(rs.getString("total"));

                    }
                    if(rs.next()) {
                        lblRe3.setText(rs.getString("first_name") + " " + rs.getString("last_name"));
                        lblCount3.setText(rs.getString("total"));
                    }
                    if(rs.next()) {
                        lblRe4.setText(rs.getString("first_name") + " " + rs.getString("last_name"));
                        lblCount4.setText(rs.getString("total"));
                    }
                    if(rs.next()) {
                        lblRe5.setText(rs.getString("first_name") + " " + rs.getString("last_name"));
                        lblCount5.setText(rs.getString("total"));
                    }
